...
#%%
fig, ax = plt.subplots(3,3,sharex=True,figsize=(10,4.5))
ymin = -5
ymax = 5
n = [7,50,100]
...
for i in range(3):
    ...
    if i == 0:
        ax[i,c].plot(tobs,uobs_8[n[i],:],'ro',fillstyle='none', 
          markersize=6,markeredgewidth=1,zorder=0)
...
line_labels = ['True','Erroneous','EnKF-S','Observations']
...
for i in range(3):
    ...
    if i == 0:
        ax[i,c].plot(tobs,uobs_12[n[i],:],'ro',fillstyle='none', 
          markersize=6,markeredgewidth=1,zorder=0)
...
for i in range(3):
    ...
    if i == 0:
        ax[i,c].plot(tobs,uobs_20[n[i],:],'ro',fillstyle='none', 
          markersize=6,markeredgewidth=1,zorder=0)
...
fig.tight_layout()
plt.show() 
fig.savefig('time_series_enkfs6.pdf',bbox_inches='tight')
fig.savefig('time_series_enkfs6.eps',bbox_inches='tight')
fig.savefig('time_series_enkfs6.png',bbox_inches='tight',dpi=300)
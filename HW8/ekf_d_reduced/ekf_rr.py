...
for k in range(1,nt+1):
    ...
    if k == oib[kobs]:        
        ...
        # forecast covariance
#        pf = dm @ sa @ sa.T @ dm.T + Q
        sf = np.hstack((dm @ sar, sqr))
        ...
        km = sfr @ B.T # A @ np.linalg.pinv(A.T @ A + R) 
...
for i in [1,0,2]:
    if i == 1:
        l3 = ax[i].plot(tobs,uobs[n[i],:],'ro',fillstyle='none', markersize=6,markeredgewidth=2,label="Observations")
    l1 = ax[i].plot(t,utrue[n[i],:],'-',color='k')
    l2 = ax[i].plot(t,uw[n[i],:],'b--')
    
        #ax[i].legend()
    l4 = ax[i].plot(t,ua[n[i],:],'g-.')
    
    ax[i].set_xlim([0,tmax])
    ax[i].set_ylabel(r'$u_{'+str(n[i]+1)+'}$')

ax[i].set_xlabel(r'$t$')
line_labels = ['True','Wrong','EKF-RR','Observations']
...
m.set_clim(vmin, vmax)
fig.colorbar(m,ax=ax[1],ticks=np.linspace(vmin, vmax, 6))
ax[1].set_title('EKF-RR')

diff = ua - utrue
cs = ax[2].contourf(T,X,diff,120,cmap='coolwarm')#,vmin=vmin,vmax=vmax)
m = plt.cm.ScalarMappable(cmap='coolwarm')
m.set_array(diff)
m.set_clim(vmin, vmax)
